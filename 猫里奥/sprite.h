...
typedef WORD        SPRITEACTION;  //碰撞动作
...
protected:
	...
	BOOL			m_bHidden;
	BOOL            m_bKill;
	BOOL            m_bIsJump;//是否跳过程
	BOOL            m_bIsDown;//是否自由下落过程
	RECT			m_rcCollision;  //碰撞矩阵
	//计算碰撞矩阵
	virtual void  CalcCollisionRect();

public:
	Sprite(Bitmap* pBitmap);
	Sprite(Bitmap* pBitmap, RECT& rcBounds, int QXSpirit, BOUNDSACTION baBoundsAction = BA_STOP);
	...
	void    SetKill(BOOL bKill) { m_bKill = bKill; };
	BOOL    IsJump() { return m_bIsJump; };
	void    SetJump(BOOL bJump) { m_bIsJump = bJump; };
	BOOL    IsDown() { return m_bIsDown; };
	void    SetDown(BOOL bDown) { m_bIsDown = bDown; };
	int     GetWidth() { return m_pBitmap->GetWidth(); };
	int     GetHeight() { return m_pBitmap->GetHeight(); };
	int     GetQXSpirit() { return m_iQXSpirit; };
};
